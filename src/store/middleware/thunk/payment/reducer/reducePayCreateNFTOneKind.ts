...
export const reducePayCreateNFTOneKind = (): AppThunk => async (dispatch, getState) => {
  ...
  const payload = JSON.parse(getState().payment.action.payload) as CreateNFTOneKind;
  ...
  const content =
    payload.state.utility === 'content' ? await uploadURItoIPFS(payload.state.contentUri) : '';
