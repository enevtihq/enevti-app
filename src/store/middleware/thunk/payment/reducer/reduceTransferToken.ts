...
export const reduceTransferToken = (): AppThunk => async (dispatch, getState) => {
  try {
    ...
    dispatch(setPaymentStatus({ action: 'transferToken', type: 'process', message: '' }));
    ...
    if (response.status === 200) {
      dispatch(setPaymentStatus({ action: 'transferToken', type: 'success', message: '' }));
    } else {
      dispatch(setPaymentStatus({ action: 'transferToken', type: 'error', message: response.data }));
    }
  } catch (err: any) {
    handleError(err);
    dispatch(setPaymentStatus({ action: 'transferToken', type: 'error', message: err.message }));
  } finally {
