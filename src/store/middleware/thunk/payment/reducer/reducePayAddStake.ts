...
export const reducePayAddStake = (): AppThunk => async (dispatch, getState) => {
  try {
    ...
    dispatch(setPaymentStatus({ action: 'addStake', type: 'process', message: '' }));
    ...
    if (response.status === 200) {
      dispatch(setPaymentStatus({ action: 'addStake', type: 'success', message: '' }));
    } else {
      dispatch(setPaymentStatus({ action: 'addStake', type: 'error', message: response.data }));
    }
  } catch (err: any) {
    handleError(err);
    dispatch(setPaymentStatus({ action: 'addStake', type: 'error', message: err.message }));
  } finally {
